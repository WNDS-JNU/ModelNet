...
def test_deduct_quota_called_for_response_aggregator_synthesis_node() -> None:
    layer = LLMQuotaLayer()
    node = MagicMock()
    node.id = "response-aggregator-node-id"
    node.execution_id = "execution-id"
    node.node_type = RESPONSE_AGGREGATOR_NODE_TYPE
    node.tenant_id = "tenant-id"
    node.require_run_context_value.return_value = _build_dify_context()
    node.model_instance, raw_model_instance = _build_wrapped_model_instance()
...
def test_response_aggregator_synthesis_quota_precheck_passes_without_abort() -> None:
    layer = LLMQuotaLayer()
    stop_event = threading.Event()
    layer.command_channel = MagicMock()
